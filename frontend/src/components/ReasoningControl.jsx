...
const REASONING_MODES = [
  { id: "off", label: "Desactivado", hint: "El modelo responde directo" },
  { id: "thinking", label: "Thinking", hint: "Razona, pero no se muestra" },
  { id: "preserve", label: "Preserve Thinking", hint: "Se muestra el bloque <think>" },
];

const EFFORT_PRESETS = [
  { id: "low", label: "Bajo", tokens: 1024 },
  { id: "medium", label: "Medio", tokens: 4096 },
  { id: "high", label: "Alto", tokens: 8192 },
  { id: "max", label: "Máx", tokens: 16384 },
];
  ...
  return (
    <div className="relative" ref={wrapRef}>
      <button
        ...
        aria-expanded={open}
        aria-haspopup="menu"
        title={active ? `Razonamiento: ${value.mode === "thinking" ? "Thinking" : "Preserve Thinking"}` : "Razonamiento desactivado"}
        className={
        ...
        >
            ...
            {REASONING_MODES.map((mode) => (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block text-glyvex-text">{mode.label}</span>
                  <span className="block text-xs text-glyvex-muted">{mode.hint}</span>
                </span>
              </button>
            ))}
          </div>
          ...
          <div className="px-3 py-2 border-b border-white/10 space-y-2">
            <span className="block text-xs text-glyvex-muted">
              Esfuerzo (budget_tokens): {value.budget === -1 ? "∞" : value.budget.toLocaleString()}
            </span>
            ...
            {customOpen && (
              <input
                ...
                placeholder="tokens (-1 = sin límite)"
              />
            )}
          </div>
